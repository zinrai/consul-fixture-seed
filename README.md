...
tool registers exactly that set and nothing else; re-running reads
the current state — at Consul's default (leader-forwarded) consistency, never
`?stale`, so it sees its own writes even on a cluster still settling replication —
and registers only what is missing.
...
- **Reviewed destination**: the write host comes from `inventory.json`'s `hosts`,
  not a command-line argument — a mistyped host can't reach production.
